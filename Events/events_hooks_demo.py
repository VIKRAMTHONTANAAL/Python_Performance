...
def email (i, j , req_id, message=None, **kwargs):
    print("Sending", message, "IN EMAIL REQUEST", req_id)

send_email_notifications.add_listener(email)

def sms_text (i, j , req_id, message=None, **kwargs):
    print("Sending", message, "IN SMS REQUEST", req_id)
...
class LoadTest(HttpUser):
    ...
    def home_page(self):
        ...
        with self.client.get("/test", name="T00_FailedHomepage", catch_response=True) as response:
            if response.status_code == 200:
